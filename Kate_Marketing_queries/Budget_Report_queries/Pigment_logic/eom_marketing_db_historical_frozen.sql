--DONT DROP THIS TABLE, saves historical data as it
CREATE TABLE cj_data_layer.eom_marketing_database_2024_to_07_2025 AS
WITH all_months AS ( --here we select all months that historical information is available for (entered marketing database gives earlies values than create_date which we need)
    ...
    UNION DISTINCT
    SELECT DISTINCT MIN(DATE_TRUNC('month', entered_marketing_database_at_test)) AS create_date
    FROM dw.hs_contact_live
...
AND am.create_date >= '2023-12-01' AND am.create_date <= '2025-07-01'
