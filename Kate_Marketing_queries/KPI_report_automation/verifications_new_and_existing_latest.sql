   WITH all_verifications AS (
    SELECT
        ...
        CASE WHEN hs.email LIKE '%gdpr%' THEN 'gdpr'
            ...
            CASE WHEN hs.country IN ('Colombia', 'Mexico', 'Italy', 'Brazil', 'Spain') AND COALESCE(hs.contact_type_segment_test, 'UNKNOWN') IN
                ...
                WHEN hs.country IN ('Italy') AND COALESCE(hs.contact_type_segment_test, 'UNKNOWN') IN ('GENERAL PRACTITIONER', 'GENERAL PRACTITIONER & DOCTOR')
                    THEN 'GP'
            ...
            CASE WHEN (hs.email IS NULL AND hs.hubspot_id IS NOT NULL) -- email null while the hs id is not null (contact exists)
                OR (hs.saas_user_type_batch IN ('Clinic Secretary Accountant', 'Clinic Secretary Marketing', 'Doctor Secretary', 'Clinic Secretary Receptionist', 'Clinic Secretary', 'Clinic Secretary Head of reception')
                    AND hs.source_doctor_id IS NULL AND hs.source_facility_id IS NULL AND hs.secretary_managed_facility_id_dwh_batch IS NOT NULL AND
                        hs.hubspot_id IS NOT NULL) -- contact exists and is a secretary
            ...
            ROW_NUMBER() OVER (PARTITION BY hs.hubspot_id, DATE_TRUNC('month', hs.new_verification_at) ORDER BY hs.dw_updated_at DESC) AS row_per_lcs,
        CASE WHEN DATE_TRUNC('month', hs.new_verification_at) = DATE_TRUNC('month', hs.entered_marketing_database_at_test) OR  hs.entered_marketing_database_at_test BETWEEN hs.new_verification_at AND dateadd('day', 5, hs.new_verification_at)
        THEN 'new' ELSE 'existing' END AS new_vs_existing_flag
        ...
        WHERE hcl.is_deleted IS FALSE AND hs.country IN ('Spain', 'Chile', 'Colombia', 'Germany', 'Italy', 'Mexico', 'Poland', 'Turkiye', 'Brazil')
    ...
    QUALIFY email_status = 'valid' AND target IN ('DOCTOR', 'GP') AND (subbrand LIKE '%Doctoralia%' OR subbrand LIKE '%MioDottore%' OR subbrand LIKE '%MioDottore%' OR subbrand LIKE '%jameda%' OR subbrand LIKE '%DoktorTakvimi%' OR subbrand LIKE '%ZnanyLekarz%' OR subbrand IS NULL)
    AND row_per_lcs = 1
       )
    ...
    SELECT
        country,
        verification_month,
        new_vs_existing_flag,
        COUNT(DISTINCT hubspot_id) FROM all_verifications
